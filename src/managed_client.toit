...
class Client:
  ...
  constructor --connector/Connector --options/SessionOptions
      ...
      --max-inflight-bytes/int=65_536
      --max-incoming/int=16
      --max-incoming-bytes/int=65_536
      --max-subscriptions/int=64
      --io-timeout/Duration=(Duration --s=10):
    if options.max-inflight > 65_000 or max-subscriptions < 1 or max-subscriptions > 256 or io-timeout.in-us <= 0:
    ...
    options_ = SessionOptions --client-id=options.client-id
        ...
        --max-inflight=options.max-inflight
    retry_ = retry
    wire_ = Wire --max-packet-size=max-packet-size
    state_ = ClientState_ options.max-inflight max-inflight-bytes max-incoming max-incoming-bytes
    timeout_ = io-timeout
